...
if not exist "%JAVA%" (
  echo Java 21 runtime not found: %JAVA%
  exit /b 1
)

echo Ensuring local MySQL 3307...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%ROOT%\scripts\start-local-mysql-3307.ps1"
if errorlevel 1 exit /b 1

echo Starting Java services...

start "pipeline-auth" /D "%ROOT%\pipeline-energy-cloud\pipeline-auth" /B cmd /c ""%JAVA%" -Dfile.encoding=UTF-8 -jar target\pipeline-auth-1.0.0-SNAPSHOT.jar --spring.datasource.url=%DB_URL% --spring.datasource.username=root --spring.datasource.password=root --spring.cloud.nacos.discovery.enabled=false --spring.cloud.nacos.config.enabled=false 1>"%LOGDIR%\pipeline-auth.out.log" 2>"%LOGDIR%\pipeline-auth.err.log""

start "pipeline-system" /D "%ROOT%\pipeline-energy-cloud\pipeline-system" /B cmd /c ""%JAVA%" -Dfile.encoding=UTF-8 -jar target\pipeline-system-1.0.0-SNAPSHOT.jar --spring.datasource.url=%DB_URL% --spring.datasource.username=root --spring.datasource.password=root --spring.cloud.nacos.discovery.enabled=false --spring.cloud.nacos.config.enabled=false 1>"%LOGDIR%\pipeline-system.out.log" 2>"%LOGDIR%\pipeline-system.err.log""

start "pipeline-data" /D "%ROOT%\pipeline-energy-cloud\pipeline-data" /B cmd /c ""%JAVA%" -Dfile.encoding=UTF-8 -jar target\pipeline-data-1.0.0-SNAPSHOT.jar --spring.datasource.url=%DB_URL% --spring.datasource.username=root --spring.datasource.password=root --spring.cloud.nacos.discovery.enabled=false --spring.cloud.nacos.config.enabled=false 1>"%LOGDIR%\pipeline-data.out.log" 2>"%LOGDIR%\pipeline-data.err.log""

start "pipeline-calculation" /D "%ROOT%\pipeline-energy-cloud\pipeline-calculation" /B cmd /c ""%JAVA%" -Dfile.encoding=UTF-8 -jar target\pipeline-calculation-1.0.0-SNAPSHOT.jar --spring.datasource.url=%DB_URL% --spring.datasource.username=root --spring.datasource.password=root --spring.cloud.nacos.discovery.enabled=false --spring.cloud.nacos.config.enabled=false 1>"%LOGDIR%\pipeline-calculation.out.log" 2>"%LOGDIR%\pipeline-calculation.err.log""
