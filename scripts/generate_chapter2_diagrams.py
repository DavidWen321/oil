...
def generate_frontend_design() -> str:
    ...
    parts.append(card(790, 360, 286, 186, "特色功能", ["故障诊断", "方案对比", "碳排核算", "实时监控"], CARD, ORANGE))
    parts.append(card(1132, 360, 286, 186, "智能助手", ["智能对话", "知识库录入", "智能报告"], CARD, PURPLE))
    parts.append(card(450, 618, 700, 154, "报告与可视化输出", ["图表趋势、告警卡片、分析结论、正式报告下载（DOCX/PDF）"], CYAN_LIGHT, CYAN))
...
def generate_architecture() -> str:
    ...
    parts.append(card(522, 180, 556, 132, "前端应用层", ["pipeline-react：登录、数据管理、计算分析、特色功能、智能助手、报告中心"], BLUE_LIGHT, BLUE))
    ...
    parts.append(card(500, 402, 360, 204, "Python 智能体服务", ["Chat / Trace / Knowledge / Report", "Graph Workflow / MCP / RAG"], PURPLE_LIGHT, PURPLE))
    parts.append(card(928, 402, 520, 204, "基础设施与平台能力", ["MySQL、Redis、Nacos", "MinIO、Milvus", "日志、配置、服务发现"], ORANGE_LIGHT, ORANGE))
    parts.append(card(214, 674, 364, 108, "专业分析结果", ["水力指标、泵站优化结果、敏感性分析、报告记录"], CYAN_LIGHT, CYAN))
    parts.append(card(648, 674, 364, 108, "知识增强结果", ["知识库问答、故障诊断建议、方案说明、报告摘要"], CARD, PURPLE))
    parts.append(card(1082, 674, 256, 108, "输出形态", ["页面图表 + 报告下载"], CARD, RED))
