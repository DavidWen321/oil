...
export default function PipelineList() {
  ...
  const columns: ColumnsType<Pipeline> = [
    ...
    },
    {
      ...
      align: 'right',
      render: (val: number) => (
        <span>{val?.toFixed(1)} <span style={{ color: 'var(--text-muted)', fontSize: 'var(--text-xs)' }}>km</span></span>
      ),
    },
    {
      title: '管径',
      ...
      align: 'right',
      render: (val: number) => (
        <span>{val} <span style={{ color: 'var(--text-muted)', fontSize: 'var(--text-xs)' }}>mm</span></span>
      ),
    },
    {
      title: '壁厚',
    ...
    },
    {
      ...
      align: 'right',
      render: (val: number) => (
        <span>{val} <span style={{ color: 'var(--text-muted)', fontSize: 'var(--text-xs)' }}>m³/h</span></span>
      ),
    },
    {
      title: '起点高程',
      ...
      align: 'right',
      render: (val: number) => (
        <span>{val} <span style={{ color: 'var(--text-muted)', fontSize: 'var(--text-xs)' }}>m</span></span>
      ),
    },
    {
      title: '终点高程',
  ...
  return (
    <AnimatedPage className={styles.page}>
      <div className={styles.pageContent}>
        <header className={styles.header}>
          <div className={styles.headerTop}>
            ...
            <div className={styles.headerActions}>
              <Button type="primary" icon={<PlusOutlined />} onClick={handleAdd} size="middle">新增管道</Button>
            </div>
          </div>
        </header>
        ...
        <Card className={styles.tableCard} bordered={false}>
          <div className={styles.toolbar}>
            <div className={styles.toolbarLeft}>
              <Input placeholder="搜索管道名称..." prefix={<SearchOutlined style={{ color: 'var(--text-muted)' }} />} value={searchText} onChange={(e) => setSearchText(e.target.value)} className={styles.searchInput} allowClear />
            </div>
            ...
            cardRender={(record) => (
              <div style={{ padding: 'var(--space-4)', background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-lg)' }}>
                ...
                <div style={{ fontSize: 13, color: 'var(--text-tertiary)', display: 'flex', flexDirection: 'column', gap: 4 }}>
                  ...
                  <span>高程: {record.startAltitude}m → {record.endAltitude}m</span>
        ...
        <Modal title={editingItem ? '编辑管道' : '新增管道'} open={modalVisible} onOk={handleSubmit} onCancel={() => setModalVisible(false)} width={680} destroyOnClose className={styles.modal} okText="保存" cancelText="取消">
          <Form form={form} layout="vertical">
            ...
            <Row gutter={16}>
              ...
              <Col span={8}><Form.Item name="diameter" label="管径 (mm)" rules={[{ required: true }]}><InputNumber min={0} style={{ width: '100%' }} /></Form.Item></Col>
              <Col span={8}><Form.Item name="thickness" label="壁厚 (mm)" rules={[{ required: true }]}><InputNumber min={0} style={{ width: '100%' }} /></Form.Item></Col>
            </Row>
            <Row gutter={16}>
              <Col span={8}><Form.Item name="throughput" label="输量 (m³/h)"><InputNumber min={0} style={{ width: '100%' }} /></Form.Item></Col>
              <Col span={8}><Form.Item name="startAltitude" label="起点高程 (m)"><InputNumber style={{ width: '100%' }} /></Form.Item></Col>
              <Col span={8}><Form.Item name="endAltitude" label="终点高程 (m)"><InputNumber style={{ width: '100%' }} /></Form.Item></Col>
            </Row>
            <Form.Item name="roughness" label="粗糙度"><InputNumber min={0} precision={4} style={{ width: '100%' }} /></Form.Item>
          </Form>
        </Modal>
      </div>
    </AnimatedPage>
