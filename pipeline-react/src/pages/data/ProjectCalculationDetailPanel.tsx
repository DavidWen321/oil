...
type ProjectCalculationDetailPanelProps = {
  project: Project;
  onClose: () => void;
};
...
const INPUT_FIELDS: FieldConfig[] = [
  { label: '流量', key: 'flowRate', unit: 'm3/h' },
  { label: '密度', key: 'density', unit: 'kg/m3' },
...
function renderDescriptions(
  title: string,
  items:
    | ReturnType<typeof buildDescriptionItems>
...
export default function ProjectCalculationDetailPanel({
  project,
  onClose,
}: ProjectCalculationDetailPanelProps) {
  ...
  const parsedHistories = useMemo(() => {
    return histories
      .map((history): ParsedHistory => ({
        ...
        input: parseJson(history.inputParams) ?? {},
        output: parseJson(history.outputResult) ?? {},
      }))
      .sort((left, right) => {
        const leftOrder = CALCULATION_TYPE_ORDER[left.calcType ?? ''] ?? Number.MAX_SAFE_INTEGER;
        const rightOrder =
          CALCULATION_TYPE_ORDER[right.calcType ?? ''] ?? Number.MAX_SAFE_INTEGER;
